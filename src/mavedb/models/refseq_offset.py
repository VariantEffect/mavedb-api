...
class RefseqOffset(Base):
    ...
    identifier : Mapped[RefseqIdentifier] = relationship("RefseqIdentifier", backref=backref("target_gene_offsets", uselist=True))
    target_gene_id = Column(Integer, ForeignKey("target_genes.id"), nullable=False, primary_key=True)
    target_gene : Mapped[TargetGene] = relationship(back_populates="refseq_offset")
