...
from mavedb.worker.jobs.external_services.clingen import submit_score_set_mappings_to_ldh
...
def main(db: Session, urns: Sequence[str], all: bool) -> None:
    """
    Submit data to ClinGen LDH for mapped variant allele ID generation for the given URNs.
    """
    db = SessionLocal()
    ...
    if all:
        ...
        logger.info(f"Command invoked with --all. Routine will submit LDH data for {len(score_set_ids)} score sets.")
    else:
        score_set_ids = db.scalars(select(ScoreSet.id).where(ScoreSet.urn.in_(urns))).all()
        logger.info(f"Submitting LDH data for the provided score sets ({len(score_set_ids)}).")

    # Unique correlation ID for this batch run
    correlation_id = f"populate_mapped_variants_{datetime.datetime.now().isoformat()}"

    # Job definition for ldh submission
    job_def = STANDALONE_JOB_DEFINITIONS[submit_score_set_mappings_to_ldh]
    ...
    for score_set_id in score_set_ids:
        logger.info(f"Submitting LDH data for score set ID {score_set_id}...")
        ...
        await submit_score_set_mappings_to_ldh(ctx, job_run.id)  # type: ignore
