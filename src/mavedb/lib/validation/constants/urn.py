...
MAVEDB_EXPERIMENT_URN_PATTERN = r"{pattern}-([a-z]+|0)$".format(pattern=MAVEDB_EXPERIMENTSET_URN_PATTERN[:-1])
MAVEDB_EXPERIMENT_URN_RE = re.compile(MAVEDB_EXPERIMENT_URN_PATTERN)

# Score set Pattern/Compiled RE
MAVEDB_SCORE_SET_URN_PATTERN = r"{pattern}-\d+$".format(pattern=MAVEDB_EXPERIMENT_URN_PATTERN[:-1])
MAVEDB_SCORE_SET_URN_RE = re.compile(MAVEDB_SCORE_SET_URN_PATTERN)

# Variant Pattern/Compiled RE
MAVEDB_VARIANT_URN_PATTERN = r"{pattern}#\d+$".format(pattern=MAVEDB_SCORE_SET_URN_PATTERN[:-1])
