...
def score_calibration_as_iri(score_calibration: ScoreCalibration) -> IRI:
    """
    Create an IRI as described in <https://datatracker.ietf.org/doc/html/rfc3986#section-4.1> for the provided MaveDB score calibration. Within
    the context of VA-Spec, these can be used interchangeably with an equivalent document object for brevity.
    """
    # TODO#XXX: Although a dedicated page for score calibrations exists, it doesn't yet have a mechanism for selecting a calibration by URN.
    # TODO(#372): nullable URN.
    return IRI(
        f"{MAVEDB_FRONTEND_URL}/score-sets/{score_calibration.score_set.urn}/calibrations?calibration={urllib.parse.quote_plus(score_calibration.urn)}"  # type: ignore
    )


def score_calibration_as_document(score_calibration: ScoreCalibration) -> Document:
    """
    Create a [VA Document](https://va-ga4gh.readthedocs.io/en/latest/core-information-model/entities/information-entities/document.html#document)
    object from the provided MaveDB score calibration.
    """
    return Document(
        ...
        extensions=[
            ...
            *(
            ...
            Extension(
                ...
                value=score_calibration.investigator_provided,
                description="Indicates whether this score calibration was provided by the investigator rather than generated by MaveDB.",
            ),
            *[
                Extension(
                    name=fc.label,
                    value=fc.range if fc.range else fc.class_,
                    description=fc.description or "No description for this functional classification provided.",
                )
                for fc in score_calibration.functional_classifications
            ],
        ],
        documentType="score calibration",
        urls=[score_calibration_as_iri(score_calibration).root],
    )
