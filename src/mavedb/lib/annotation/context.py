...
class VariantAnnotationContext:
    ...
    ``measured_allele`` is the authoritative allele (the assayed representation); its ``post_mapped`` VRS is
...
def variant_annotation_context(
    ...
    Returns ``None`` when the variant is unmapped at ``as_of`` — no live mapping record, or an authoritative
    allele that carries no ``post_mapped`` VRS (nothing to annotate). One record fetch + one allele-link
    fetch; the Cat-VRS transit is built from the same links, so the subject costs no redundant query.
    """
    record = db.scalar(
    ...
    # (include_convergent=False): the synonymous cousins are dropped, because VA-Spec carries no per-member
