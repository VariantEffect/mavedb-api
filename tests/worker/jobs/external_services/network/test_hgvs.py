...
class TestE2EHgvsMappingJobs:
    ...
    async def test_hgvs_mapping_jobs_e2e(
        self,
        session,
        arq_redis,
        arq_worker,
        sample_score_set,
        with_submit_hgvs_mapping_jobs_pipeline,
        sample_submit_hgvs_mapping_jobs_pipeline,
        ...
        # Enqueue the HGVS mapping job
        await arq_redis.enqueue_job(
            "submit_hgvs_mapping_jobs_for_score_set", sample_submit_hgvs_mapping_jobs_run_in_pipeline.id
        )
        ...
        session.refresh(sample_submit_hgvs_mapping_jobs_run_in_pipeline)
        assert sample_submit_hgvs_mapping_jobs_run_in_pipeline.status == JobStatus.SUCCEEDED

        # Verify that job metadata contains processing information
        metadata = sample_submit_hgvs_mapping_jobs_run_in_pipeline.metadata_
        ...
        session.refresh(sample_submit_hgvs_mapping_jobs_pipeline)
        assert sample_submit_hgvs_mapping_jobs_pipeline.status == PipelineStatus.SUCCEEDED
    ...
    async def test_hgvs_mapping_jobs_metadata_tracking(
        self,
        session,
        arq_redis,
        arq_worker,
        ...
        """Test that HGVS mapping jobs properly track metadata."""
    ...
    async def test_hgvs_mapping_jobs_progress_reporting(
        self,
        session,
        arq_redis,
        arq_worker,
        ...
        """Test that HGVS mapping jobs properly report progress."""
        ...
        context = sample_submit_hgvs_mapping_jobs_run_in_pipeline.context_
        assert context is not None
        assert context.get("application") == "mavedb-worker"
        assert context.get("function") == "submit_hgvs_mapping_jobs_for_score_set"
