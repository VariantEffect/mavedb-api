...
def test_users_get_one_published_score_set_from_other_experiment(
...
def test_users_get_one_published_score_set_from_others_experiment_with_a_private_score_set(
    ...
    pub_score_set = publish_score_set(client, score_set_1["urn"], score_set_id)
...
def test_search_meta_analysis_experiment(session, data_provider, client, setup_router_db, data_files):
    ...
    score_set_id = session.scalars(select(ScoreSetDbModel.id).where(ScoreSetDbModel.urn == meta_score_set["urn"])).one()
    published_meta_score_set = publish_score_set(client, meta_score_set["urn"], score_set_id)
...
def test_search_exclude_meta_analysis_experiment(session, data_provider, client, setup_router_db, data_files):
    ...
    score_set_id = session.scalars(select(ScoreSetDbModel.id).where(ScoreSetDbModel.urn == meta_score_set["urn"])).one()
    meta_score_set = publish_score_set(client, meta_score_set["urn"], score_set_id)
